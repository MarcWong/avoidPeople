...
bool DECOFUNC(processMultiInputData)(void * paramsPtr, void * varsPtr, QVector<QVector<void *> > inputParams, QVector<QVector<void *> > inputData, void * outputData, QList<int> & outputPortIndex)
{
    ...
    */
//此处需完成！
//inputdata_0即Deadreckoning模块输出，数据结构参见该模块
...
//inputdata_1->data[0]即角度为0的激光束的测距值，inputdata_1->data[360]即为角度为180度的激光束的测距值
//激光坐标系
//         机器人面朝方向
//               |
//               |
...
//表示该处存在障碍物的得票数，参见课件简易投票法
    ...
    for(int i = 0; i < 361; i++)
    {
        double dis = inputdata_1.front()->data[i];
        if (dis==0)
            continue;
        double theta = (double(i) * 0.5)*PI/180;
        double alpha = inputdata_0.front()->orientation - PI/2 + theta;
        double gx = inputdata_0.front()->x + dis*cos(alpha)/100;
        double gy = inputdata_0.front()->y + dis*sin(alpha)/100;
        int iy = int((gy)/params->mapRes) - params->zeroY;
        int ix = int((gx)/params->mapRes) - params->zeroX;
        int step = 5, threshold = 200;
        if (ix>=0 && ix<params->mapWidth && iy>=0 && iy<params->mapHeight)
            if (vars->map[iy][ix] + step <=threshold)
                vars->map[iy][ix] += step;
            else
                vars->map[iy][ix] = 255;
    }
